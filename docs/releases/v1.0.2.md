...
- 新增 `under-utils-ai` 与 `under-utils-ai-starter`，支持 OpenAI-compatible 同步文本对话、SSE 流式响应、provider 扩展、响应元数据、基础错误分类、token 用量和敏感信息脱敏。
...
当前草稿最近一次本地验证时间：`2026-05-31 15:55 Asia/Shanghai`。已通过：

- `git diff --check`
- `/usr/local/apache-maven-3.9.9/bin/mvn -q -pl under-utils-ai,under-utils-ai-starter,under-utils-redis,under-utils-samples -am test`
